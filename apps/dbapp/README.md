...
This is a sample template for dbapp - Below is a brief explanation of what we have generated for you:
...
the role for the lambda needs AWSLambdaVPCAccessExecutionRole as well as log access
must participate in the vpc if the rds is limited to traffic from your vpc
AmazonSSMReadOnlyAccess
Basic Log access (<http://donhenton.com/blog/archives/2561>) includes policy for log access
...
## Using AWS SAM
 ...
 if an error occurs where packaging zips about dates before 1980
 run the command below to update the files

```bash
 find dbcode/node_modules/ -mtime +10950 -exec touch {} +
```
...
* use nvm aws-10 on this machine
...
## Sending env variables into local runs
...
```bash
sam logs -n HelloWorldFunction --stack-name dbapp --tail
```
...
```bash
aws cloudformation delete-stack --stack-name dbapp
```
